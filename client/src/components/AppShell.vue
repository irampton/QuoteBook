<template>
  <div class="min-h-screen bg-cream">
    <div :inert="libraryDialogOpen" :aria-hidden="libraryDialogOpen ? 'true' : undefined"><AppSidebar :user="user" :categories="categories" :active-category="view === 'library' ? category?.id || category?.name : null" :all-quotes-active="view === 'library' && !category && !search" :open="sidebarOpen" @close="closeSidebar" @logout="$emit('logout')" @add="showAdd" @category="showCategory" @search="showSearch" @new-category="openCategoryModal" /></div>
    <main :inert="sidebarOpen && isMobile" :aria-hidden="sidebarOpen && isMobile ? 'true' : undefined" class="min-h-screen md:ml-[286px]">
      <div :inert="libraryDialogOpen" :aria-hidden="libraryDialogOpen ? 'true' : undefined" class="sticky top-0 z-20 flex h-16 items-center border-b border-line bg-cream/90 px-5 backdrop-blur md:hidden"><button ref="menuButton" class="rounded-lg p-2" aria-label="Open menu" :aria-expanded="sidebarOpen" @click="sidebarOpen = true"><Menu :size="21"/></button><span class="ml-3 font-serif text-lg font-semibold">Quotebook</span></div>
      <div ref="content" class="px-5 py-8 outline-none sm:px-8 sm:py-10 lg:px-12 xl:px-16" tabindex="-1"><AddQuotePage ref="addQuotePage" v-show="view === 'add'" :categories="categories" @quote-saved="onQuoteSaved"/><QuoteLibrary v-if="view === 'library'" :category="category" :search="search" :refresh-key="refreshKey" :categories="categories" @add="showAdd" @changed="refreshCategories" @modal-change="libraryDialogOpen = $event"/></div>
    </main>
...
const categories = ref([...props.initialCategories]), view = ref('library'), category = ref(null), search = ref(''), sidebarOpen = ref(false), isMobile = ref(false), menuButton = ref(null), content = ref(null), addQuotePage = ref(null), refreshKey = ref(0), libraryDialogOpen = ref(false), newCategoryOpen = ref(false), categoryDialog = ref(null), newCategoryName = ref(''), categorySaving = ref(false), categoryError = ref('')
...
function showAdd() { const wasOpen = sidebarOpen.value; addQuotePage.value?.resetIfComplete(); view.value = 'add'; category.value = null; search.value = ''; sidebarOpen.value = false; focusContentIfMobile(wasOpen) }
...
async function refreshCategories() { try { const result = await api.categories(); categories.value = result?.categories || result?.items || (Array.isArray(result) ? result : categories.value) } catch {} finally { refreshKey.value++ } }
