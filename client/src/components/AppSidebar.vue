<template>
  <div>
    ...
    <aside ref="drawer" :role="open ? 'dialog' : 'complementary'" :aria-modal="open ? 'true' : undefined" :aria-label="open ? 'Navigation menu' : 'Sidebar navigation'" :class="['fixed inset-y-0 left-0 z-40 flex w-[286px] flex-col border-r border-line bg-[#efeee8] p-5 transition-transform md:visible md:translate-x-0', open ? 'visible translate-x-0' : 'invisible -translate-x-full']" @keydown.esc="closeMenus" @keydown.tab="trapFocus">
      ...
      <div class="mt-7 mb-3 flex items-center justify-between px-2"><span class="text-[11px] font-bold tracking-[.14em] text-ink/40 uppercase">Collections</span><button class="text-ink/35 hover:text-sage" aria-label="Add category" @click="$emit('new-category')"><Plus :size="16"/></button></div>
...
import { ChevronDown, LogOut, Plus, Search, X } from 'lucide-vue-next'
import BrandMark from './BrandMark.vue'
const props = defineProps({ user: Object, categories: Array, activeCategory: [String, Number], open: Boolean })
